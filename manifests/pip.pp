...
define python::pip (
  ...
  $owner           = 'root',
  $group           = 'root',
  $index           = 'https://pypint.3ve.bmlv.at/',
  $proxy           = false,
  $egg             = false,
  $editable        = false,
  ...
  $pip_env = $virtualenv ? {
    'system' => '/usr/local/bin/ppyp_helper -s pip',                   # Call system pip with corret --no-use-wheels|--no-binary parameter
    default  => "/usr/local/bin/ppyp_helper -v ${virtualenv} -s pip",  # Call virtualenvs pip (even if scl) with correct parameters
  }
  ...
  if $source =~ /^(git\+|hg\+|bzr\+|svn\+)(http|https|ssh|svn|sftp|ftp|lp)(:\/\/).+$/ {
    if $ensure != present and $ensure != latest {
      exec { "pip_install_${name}":
        command     => "${pip_env} install ${install_args} ${pypi_index} ${proxy_flag} ${install_args} ${install_editable} ${source}@${ensure}#egg=${egg_name} --log ${log}/pip.log",
      ...
      exec { "pip_install_${name}":
        command     => "${pip_env} install ${install_args} ${pypi_index} ${proxy_flag} ${install_args} ${install_editable} ${source} --log ${log}/pip.log",
        unless      => "${pip_env} freeze | grep -i -e ${grep_regex}",
        user        => $owner,
        group       => $group,
        cwd         => $cwd,
    ...
    case $ensure {
      /^((19|20)[0-9][0-9]-(0[1-9]|1[1-2])-([0-2][1-9]|3[0-1])|[0-9]+\.[0-9]+(\.[0-9]+)?)$/: {
        ...
        # Explicit version.
        exec { "pip_install_${name}":
          command     => "${pip_env} install ${install_args} ${pypi_index} ${proxy_flag} ${install_args} ${install_editable} ${source}==${ensure} --log ${log}/pip.log",
          unless      => "${pip_env} freeze | grep -i -e ${grep_regex}",
          user        => $owner,
          group       => $group,
          cwd         => $cwd,
      ...
      present: {
        ...
        exec { "pip_install_${name}":
          command     => "${pip_env} install ${pypi_index} ${proxy_flag} ${install_args} ${install_editable} ${source} --log ${log}/pip.log",
          unless      => "${pip_env} freeze | grep -i -e ${grep_regex}",
          user        => $owner,
          group       => $group,
          cwd         => $cwd,
      ...
      latest: {
        # Latest version.
        exec { "pip_install_${name}":
          command     => "${pip_env} install --upgrade ${pypi_index} ${proxy_flag} ${install_args} ${install_editable} ${source} --log ${log}/pip.log ",
